...
import { mqtt, db, influx, notifications } from '../services';
 ...
 * params: @bike -> bike id
 */
router.get('/api/activities/last/:bike', [check('bike').isString()], async (req: any, res: any) => {
...
router.get('/api/weather/last', (_: any, res: any) => {
 ...
 */
router.get('/api/weather/last/:station', [check('station').isString()], (req: any, res: any) => {
  const err = validationResult(req);
  if (!err.isEmpty()) {
    return res.status(422).json({ err: err.array() });
  }
...
router.get('/api/alice/config', async (_: any, res: any) => {
  try {
...
router.put('/api/alice/tokens', [check('token').isString()], async (req: any, res: any) => {
  const err = validationResult(req);
  if (!err.isEmpty()) {
    return res.status(422).json({ err: err.array() });
  }
...
/* Retrive comments */
router.get('/api/alice/comments', async (_: any, res: any) => {
  try {
...
/* Delete all comments */
router.delete('/api/alice/comments', checkIfAdmin, async (_: any, res: any) => {
  try {
...
router.get('/api/alice/notifications', async (_: any, res: any) => {
  try {
 ...
 */
router.get('/api/alice/notifications/subscribers', checkIfAdmin, async (_: any, res: any) => {
  try {
